...
\section{Enviroment Setup and Import data files}
...
#create a dataframe to store all the mean squred error results as this is the
...
\section{Model interms of fit and significance}
...
\subsection{Further model investigation for heteroskedasticity & multicollinearity}
```{r}
bptest(lm.fit)
vif(lm.fit)
```
...
\begin{enumerate}
\itemsep0em 
\item rm  <-> dis: average number of rooms per dwelling & weighted mean of distances to five Boston employment centres
\item dis <-> rad: weighted mean of distances to five Boston employment centres & index of accessibility to radial highways
\item tax <-> lstat: full-value property-tax rate & lower status of the population (percent).
\item nox <-> rad: nitrogen oxides concentration & index of accessibility to radial highways
\item crim <-> lstat: per capita crime rate by town & lower status of the population (percent).
\item lstat <-> rm: lower status of the population (percent) & average number of rooms per dwelling
\end{enumerate}
```{r echo=TRUE}
...
\subsection{Cross validation controller}
...
modelPreformance <- rbind(modelPreformance,
                          data.frame(modelName="Stepped",
                                     ...
                                     mse.test=mse.value,
                                     interactionTerms=as.numeric(length(coef(stepped3)))
                                    ))
```
Test MSE for Stepped model with third order interaction terms: `r mse.value`.
...
```
\subsection{Implement using the rpart package}
